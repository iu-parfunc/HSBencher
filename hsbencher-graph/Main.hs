...
data Flag = ShowHelp
          ...
          | InverseY 
            -- TODO: deprecate InverseRatio in favor of this
...
--     , Option []    ["barclusters"] (NoArg (RenderMode BarClusters)) "Plot data as bar clusters"
     ...
     , Option []    ["inverse"] (NoArg InverseY)
                      (unlines
                      [ "Report 1/Y instead of Y on the Y-axis.  This changes slowdown"
                      , "factors to speed-up factors, and is meant for use with --factor."
                      ])
...
main = do
  ...
  let (options, inFiles, unrec,errs) = getOpt' Permute core_cli_options args
      ...
      inverseFlags      = [ x | x@InverseY{} <- options]
  ...
  let series1 :: [(Key,[LinePoint])]
      ...
      plot_series0 =
          ...
          case (normaliseKeyFlags ++ ratioFlags ++ inverseRatioFlags ++ inverseFlags) of
            [NormaliseKey{}] -> normalise base series2
            [NormaliseKey{}, InverseY] -> mapPoints (\_k pt -> onY (\y -> NumData (1.0 / fromData y)) pt) $ 
                                            normalise base series2
